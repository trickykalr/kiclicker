    ...
    function loop() {
        ...
        if (button) {
            ...
            autoCursors.forEach((c, i) => {
                ...
                // Orbite elliptique autour de l'image
                const orbitX = Math.cos(curAngle) * 230;
                const orbitY = Math.sin(curAngle) * 140;
                c.style.left = (centerX + orbitX - 25) + "px";
                c.style.top  = (centerY + orbitY - 25) + "px";
                // Flip selon la direction
                c.style.transform = orbitX > 0 ? "scaleX(-1)" : "scaleX(1)";
                // Légère variation de taille selon la position (effet 3D)
                const scale = 0.85 + 0.3 * ((Math.sin(curAngle) + 1) / 2);
                c.style.transform += ` scale(${scale.toFixed(2)})`;
                // Opacité selon profondeur
